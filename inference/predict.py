...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
    ...
    # check input folder integrity(这个caseids只是举例)
    ...
    #将caseid中，output里面有的全部排除（allfiles才是使用的所有input）
    # case_ids_finished=[]
    # if not overwrite_existing:
    #     finished_files=os.listdir(output_folder)
    #     for label in finished_files:
    #         if label.endswith('.nii.gz'):
    #             labels_caseid=label.replace('.nii.gz', '_0000.nii.gz')
    #             case_ids_finished.append(labels_caseid)
    #     all_files=[x for x in all_files if x not in case_ids_finished]
    # print('*************')
    # print(len(all_files))
    #修改完成
