...
def muilt_sliding_window_inference_3d(inputs: torch.Tensor, crop_size: Tuple[int],  model: nn.Module, outputs_size: torch.Size,outputs_size2: torch.Size, is_softmax: bool, overlap: float = 0.5) -> torch.Tensor:
    num_spatial_dims = len(inputs.shape) - 2
    if overlap < 0 or overlap >= 1:
        raise ValueError("overlap must be >= 0 and < 1.")
    ...
    slices = [[slice(None), slice(None)]+[slice(s, s + crop_size[d])
                                          for d, s in enumerate(x)] for x in out]
    outputs1 = torch.zeros(outputs_size).to(inputs.device)
    count1 = torch.zeros(outputs_size).to(inputs.device)
    outputs2 = torch.zeros(outputs_size2).to(inputs.device)
    count2 = torch.zeros(outputs_size2).to(inputs.device)
    outputs3 = torch.zeros(outputs_size2).to(inputs.device)
    count3 = torch.zeros(outputs_size2).to(inputs.device)
    ...
    for slice_s in slices:
        pred, model_feature, prior_feature = model(inputs[slice_s])
        if is_softmax:
            pred = F.softmax(pred, dim=1)
            model_feature = F.softmax(model_feature, dim=1)
            prior_feature = F.softmax(prior_feature, dim=1)
        else:
            pred = torch.sigmoid(pred)
            model_feature = torch.sigmoid(model_feature)
            prior_feature = torch.sigmoid(prior_feature)
        outputs1[slice_s] += pred.clone()
        count1[slice_s] += 1
        outputs2[slice_s] += model_feature.clone()
        count2[slice_s] += 1
        outputs3[slice_s] += prior_feature.clone()
        count3[slice_s] += 1



# pred, model_feature, prior_feature
    return outputs1 / count1,outputs2 / count2,outputs3 / count3
